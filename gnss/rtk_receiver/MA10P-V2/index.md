...
| MA10P-V2-CLAS           | GPS/QZSS/Galileo, QZSS/CLAS service, only available in Japan|
...
This page is for the MA10P-V2 product only. For the MA-10P series, please refer to [this page](../MA-10P-V1/).
...
The above data is generated by [PocketSDR 4CH](https://www.datagnss.jp/products/pocketsdr-gnss-receiver).

### Data output

By default, NMEA sentences are output over USB serial （SERIAL-A） and BLE. 

When `Forward QZS-6C data to External port` is enabled on the Advanced page, QZS-6C L6 data can be output via the External Port (6P JST).

#### Serial port

The USB baud rate is 230400 bps. Be sure to select the port labeled `SERIAL-A` (check the COM port in Device Manager).
...
L6 correction data can also be output through the 6P UART. In that case, configure `Forward QZS6C Data to External Port` on the advanced.html, if AP mode, please log in  `http://192.168.4.1/advanced.html` page.
